...
description: Use this skill when the user asks to create, rewrite, optimize, debug, review, or turn an LLM prompt into a reusable prompt template, especially when they need clear inputs, variables, constraints, output structure, failure behavior, examples, or prompt-level tests. Do not use it for Agent Skill/SKILL.md authoring or whole-agent evaluation/readiness.
---
...
If the user only wants a quick rewrite, lead with the improved prompt. Do not force the full contract format when it would add noise.
